"""Shared helpers for Zastupitelstvo města Hradec Králové's analysis runners (hradec-kralove/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per hradec-kralove/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`hradec-kralove/data/votes.csv` and `hradec-kralove/data/vote_events.json` are always already present (written by
`hradec-kralove/scripts/standardize.py`).
"""
