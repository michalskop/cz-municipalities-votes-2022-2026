"""Detect which WPCA dimension is the government/opposition axis for Zastupitelstvo města Hradec Králové.
...
`hradec-kralove/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`hradec-kralove/data/memberships.csv`) -- WITH ONE DELIBERATE DIFFERENCE, ported from Ústí nad
Labem's copy: this copy also unions in `government_members` (named individual person ids), not just
`government_groups` (whole-klub org ids). Every city's copy before Ústí only expanded
government_groups, because none of them needed a non-empty government_members list. Hradec
Králové's D7-approved govity_definition.json needs it too: 3 named coalition defectors (Dvořáková,
Outlý, Štayrová) are formally 'NEZ' (nezařazení), not covered by a klub-level group — the
groups-only logic would silently exclude them from the WPCA axis-detection government label,
contradicting govity.py itself (which already reads both fields, see
legislature-data-analyses/govity/govity.py's `is_in_government()`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`hradec-kralove/analyses/wpca/outputs/government_axis.json`.

Usage:
  python hradec-kralove/scripts/detect_government_axis.py
"""
