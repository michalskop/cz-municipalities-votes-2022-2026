"""Standardize Hradec Králové's zastupko.fit.vutbr.cz nested JSON feed into dt-standard tables.

Source: hradec-kralove/config/sources.yml's `zastupko_current` entry — a single JSON document
(dataset id 9, term 2022-2026) shaped as municipalita -> politickeSubjekty -> zastupitele ->
zastupitelstva[0] (koalice, lidr, zasedani[] -> hlasovani[] -> zastupiteleHlasy[]). Same shape as
Brno's/Most's own feeds on this backend — see sources.yml for the full schema trail and the one
open caveat (a ~1-year staleness gap, not something this file can fix).

Scope boundary (matches every other zastupko-network city's C2/C9 precedent): this standardizer
builds ONLY the bare assembly organization + membership. It deliberately does NOT build
party/coalition organizations or memberships from the feed's `politickeSubjekty`/`koalice`/`lidr`
data, even though that data is real and live-sourced with genuine since/until intervals — that
belongs to C4/D7, not here.

Unlike Brno, this feed (as of the 2026-08-30 snapshot) is CLEANER on every axis Brno needed special
handling for:
- No G5 namesake collisions in the 40-person roster (checked at C9 time; still re-verified here on
  every run in case a future snapshot adds a colliding name).
- No corrupted/unmapped `hlas` values found (Brno had one anomalous event with 48 "T"/secret-ballot
  entries) — the OPTION_MAP/skip-and-log path is kept anyway as a safety net, not removed, in case
  a future snapshot introduces one.
- No `urlProtokol` values populated on ANY event (0/1627, vs. Brno's some-but-unreachable) — so
  there is no protocol-page cross-check function here at all (Brno's `run_protocol_cross_check`
  would always be a no-op against this feed; omitted rather than kept as dead code).
- `platne` is `True` on every event, `blokove`/`tajne` are always `null` — the invalid-vote/
  procedural-flag handling is kept for schema robustness but has never actually been exercised
  against this feed.

G2 signal here (same as Brno, since `sumarizace` is likewise never populated — 0/1627): a
self-consistency check between the source's own published `prijato` (pass/fail) boolean and the
majority direction of our recomputed per-person tally. Ties (yes == no) are skipped (ambiguous, no
gate signal). Any mismatch is logged individually — see `_classify_result_consistency`.
"""
...
ORG_ID = "hradec-kralove:org:zastupitelstvo-mesta-hradec-kralove"
ORG_NAME = "Zastupitelstvo města Hradec Králové"

# schema-CZ.json's `hlas` enum has 7 values; only these 5 have ever been observed in this feed's
# live data (checked across all 1627 events / 60,199 per-person entries at C9 time). Deliberately
# NOT mapping "O" (Omluven/a, excused) or "T" (Tajná, secret ballot) — if either ever appears, fail
# loudly (skip + log) rather than guess a dt-standard option, same convention as every other
# zastupko-network city.
OPTION_MAP = {
    "A": "yes",
    "N": "no",
    "Z": "abstain",
...
def _build_organization(source_url: str) -> dict[str, Any]:
    return {
        ...
        "identifiers": json.dumps(
            [{"scheme": "hradec-kralove:org_abbr", "identifier": "ZM HK"}], ensure_ascii=False
        ),
        "sources": json.dumps(
            [{"url": source_url, "note": "zastupko.fit.vutbr.cz feed, dataset id 9, term 2022-2026"}],
            ensure_ascii=False,
        ),
    }
...
def _build_persons(
    ...
    """Returns (persons rows, {idZastupitel: person_id}). G5 namesake collisions (none found in
    this feed at C9 time) are handled by suffixing the source's own numeric id onto every
    colliding slug, never merging two distinct people — same convention as Brno."""
    ...
    for z in zastupitele:
        ...
        if len(colliding_ids) > 1:
            ...
            person_slug = slug
        person_id = f"hradec-kralove:person:{person_slug}"
        id_to_person_id[z["id"]] = person_id

        identifiers = [{"scheme": "hradec-kralove:zastupko_id", "identifier": str(z["id"])}]
        for alias in z.get("aliasy") or []:
            former = f"{alias.get('jmeno', '')} {alias.get('prijmeni', '')}".strip()
            if former:
                identifiers.append({"scheme": "hradec-kralove:former_name", "identifier": former})
        ...
        persons.append(
            {
                ...
                "sources": json.dumps(
                    [{"url": source_url, "note": f"zastupko.fit.vutbr.cz idZastupitel={z['id']}"}],
                    ensure_ascii=False,
                ),
            }
        )
...
def _build_memberships(
    ...
    for zastupko_id, dates in event_dates_by_person.items():
        ...
        memberships.append(
            {
                "id": f"hradec-kralove:membership:{person_id.split(':', 2)[2]}:{ORG_ID}",
                "person_id": person_id,
                    ...
                    [
                        {
                            ...
                            "note": (
                                ...
                                "of this feed's last recorded session — NOTE this feed's last "
                                "recorded session is 2025-08-26, ~1 year stale as of the 2026-08-30 "
                                "build; see sources.yml's known_staleness_gap."
                            ),
                        }
                    ],
                    ensure_ascii=False,
...
def _session_date(session: dict[str, Any]) -> str:
    """A session's date. This feed always uses `datum_od`/`datum_do` (the zastupko.fit.vutbr.cz
    origin server's shape, same as Brno's/Most's origin — see sources.yml's schema note); the
    documented-spec `datum` field is checked first anyway for forward compatibility, same as
    Brno's standardize.py. Neither present is a real schema break, not something to guess past —
    fail loudly."""
    if "datum" in session:
...
def _build_votes_events_motions(
    ...
    for session in zasedani:
        ...
        for h in session["hlasovani"]:
            ...
            hlasovani_id = h["id"]
            vote_event_id = f"hradec-kralove:vote-event:{hlasovani_id}"
            motion_id = f"hradec-kralove:motion:{hlasovani_id}"
            ...
            if corrupted_ids:
                data_quality["corrupted_hlas_values"] = {
                    "affected_person_source_ids": corrupted_ids,
                    "note": (
                        "Source has an unmapped hlas value for these people on this event — see "
                        "standardize.py's module docstring. No vote recorded for them here; never "
                        "fabricated."
                    ),
                }
