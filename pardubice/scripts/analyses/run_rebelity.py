"""Run the rebelity analysis for Zastupitelstvo města Pardubice.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `pardubice/analyses/rebelity/rebelity_definition.json`.
Depends on the real `pardubice:org:group:*` data from `pardubice/scripts/party_affiliation.py` (live klub
membership, not a candidate-list fallback) via `pardubice/scripts/build_all_members.py`'s "groups"
field — no political judgment needed for rebelity itself (see the definition's own approval_status).

Usage:
  python pardubice/scripts/analyses/run_rebelity.py --script /path/to/legislature-data-analyses/rebelity/rebelity.py
"""
