"""Shared helpers for Ostrava's analysis runners (ostrava/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per ostrava/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`ostrava/data/votes.csv` and `ostrava/data/vote_events.json` are always already present (written by
`ostrava/scripts/standardize.py`).
"""
