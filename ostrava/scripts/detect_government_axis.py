"""Detect which WPCA dimension is the government/opposition axis for Ostrava.
...
`ostrava/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`ostrava/data/memberships.csv`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`ostrava/analyses/wpca/outputs/government_axis.json`.

Usage:
  python ostrava/scripts/detect_government_axis.py
"""
