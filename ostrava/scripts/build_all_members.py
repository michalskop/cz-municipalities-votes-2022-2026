"""Build an all-members.dt.analyses-shaped JSON from Ostrava's standard tables.
...
Ostrava's `ostrava/scripts/party_affiliation.py` populates real `classification: "group"`
organizations/memberships derived from dated per-vote klub groupings (D7's preferred case, same
treatment as Brno's real live data — see that script's module docstring for the real, dated ANO
club split it captures, and the known post-2025-06-18 data gap). This script reads those into
`memberships.groups` only — no dual-write into `memberships.candidate_list`, since that field is
meant for genuine candidate-list-origin data, which this isn't.

Output is NOT committed (ostrava/work/ is gitignored) — regenerate on demand before running an
