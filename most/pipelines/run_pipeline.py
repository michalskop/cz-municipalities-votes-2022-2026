"""Most pipeline orchestrator: download -> standardize -> validate (G1) -> cross-check (G2).

Usage:
    python most/pipelines/run_pipeline.py
    python most/pipelines/run_pipeline.py --skip-download   # reuse the last downloaded JSON

Ported directly from brno/pipelines/run_pipeline.py (Most's standardize.py has the same
raw_path/out_dir/sources_path signature, same report shape, same protocol cross-check function) —
see that file's docstring for the general rationale. Only the module-level docstring and the
mismatch-rate bound rationale below are Most-specific.
...
No party-affiliation/coalition step here (matches Brno/Ostrava's C2 precedent): Most's per-vote
party grouping hasn't been investigated yet — that's C4's job, needing owner sign-off (D7), not
C9's.
"""
import argparse
import json
...
def run(skip_download: bool = False, protocol_cross_check_sample: int = 10) -> dict:
    ...
    # Bound: mirrors Brno's 5% ceiling. Most's own first run (2026-08-27) came back at 0% (489/489
    # match) — far below this — so this ceiling exists purely as a future-regression guard, not
    # because any real exceptions are currently expected here.
    ...
    logging.info("Most pipeline finished: G1 and G2 gates passed.")
    return report
