"""Run the rebelity analysis for Most.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `most/analyses/rebelity/rebelity_definition.json`.
Depends on the real `most:org:group:*` data from `most/scripts/party_affiliation.py` (live klub
membership, not a candidate-list fallback) via `most/scripts/build_all_members.py`'s "groups"
field — no political judgment needed for rebelity itself (see the definition's own approval_status).

Usage:
  python most/scripts/analyses/run_rebelity.py --script /path/to/legislature-data-analyses/rebelity/rebelity.py
"""
