"""Shared helpers for Most's analysis runners (most/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per most/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`most/data/votes.csv` and `most/data/vote_events.json` are always already present (written by
`most/scripts/standardize.py`).
"""
