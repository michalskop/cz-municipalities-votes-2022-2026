"""Detect which WPCA dimension is the government/opposition axis for Most.
...
`most/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`most/data/memberships.csv`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`most/analyses/wpca/outputs/government_axis.json`.

Usage:
  python most/scripts/detect_government_axis.py
"""
