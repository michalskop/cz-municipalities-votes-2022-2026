"""Run the attendance analysis for Praha.

Invokes the shared, unmodified `attendance.py` from the separate `legislature-data-analyses`
repository (see `_common.py`'s module docstring for why `--script` has no default). City-specific
content is entirely in `praha/analyses/attendance/attendance_definition.json`.

PROVISIONAL OUTPUT: as of this runner's introduction (C8, 2026-08-04), attendance_definition.json
is a Sonnet-drafted DRAFT that has not yet had project-owner sign-off (plan.md D7). This runner is
safe to build/test (attendance has no government_groups dependency, so it's lower-risk than
govity), but its output must not be wired into a nightly workflow or the dashboard until the
owner has reviewed the underlying data (see praha/analyses/README.md).
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-build-persons",
        action="store_true",
        help="reuse the existing --persons file instead of regenerating it from praha/data/",
    )
