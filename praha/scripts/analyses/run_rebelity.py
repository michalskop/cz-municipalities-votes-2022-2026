"""Run the rebelity analysis for Praha.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in
`praha/analyses/rebelity/rebelity_definition.json`.

PROVISIONAL OUTPUT: rebelity_definition.json is a Sonnet-drafted DRAFT (plan.md D7) — see
praha/analyses/README.md. Depends on the `praha:org:candidate-list:*` group data from
`praha/scripts/party_affiliation.py` (also unreviewed) via `praha/scripts/build_all_members.py`'s
"groups" field. Safe to build/test; do not wire into a nightly workflow or the dashboard yet.
