"""Detect which WPCA dimension is the government/opposition axis for Brno.
...
`brno/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`brno/data/memberships.csv`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`brno/analyses/wpca/outputs/government_axis.json`.

Usage:
  python brno/scripts/detect_government_axis.py
"""
