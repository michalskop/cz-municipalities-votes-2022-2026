"""Build an all-members.dt.analyses-shaped JSON from Brno's standard tables.
...
Unlike Praha (which has no live klub data and falls back to `classification: "candidate_list"`,
see praha/scripts/build_all_members.py's docstring), Brno's `brno/scripts/party_affiliation.py`
populates REAL, live `classification: "group"` organizations/memberships directly from the feed —
D7's preferred case. This script reads those into `memberships.groups` only — no dual-write into
`memberships.candidate_list`, since that field is meant for genuine candidate-list-origin data,
which this isn't.

Output is NOT committed (brno/work/ is gitignored) — regenerate on demand before running an
