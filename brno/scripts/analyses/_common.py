"""Shared helpers for Brno's analysis runners (brno/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per brno/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`brno/data/votes.csv` and `brno/data/vote_events.json` are always already present (written by
`brno/scripts/standardize.py`).
"""
