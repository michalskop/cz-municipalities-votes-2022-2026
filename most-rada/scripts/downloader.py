"""Download Most's RADA (executive council) roll-call vote JSON feed to most-rada/work/raw/.

Source and format notes: most-rada/config/sources.yml. Same shared FIT VUT `zastupko.cz` backend
already used by most/'s zastupitelstvo pipeline (organ="rada" instead of "zastupitelstvo",
dataset id 8 = the 2022-2026 term). The feed is one JSON response covering the whole term to
date — always re-fetch, never trust a stale copy.

IPv4 + retry: ported directly from most/scripts/downloader.py (itself ported from Brno's
already-hardened version — see that file's docstring for the two real incidents that motivated
each piece).
"""
import argparse
import contextlib
...
_DEFAULT_OUT = _CITY_ROOT / "work" / "raw" / "zastupko_rada_dataset_8.json"
