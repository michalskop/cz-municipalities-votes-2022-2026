"""Standardize Most's RADA (executive council) zastupko.cz feed into dt-standard tables.

Source: most-rada/config/sources.yml's `zastupko_current` entry — same shape and same shared FIT
VUT backend as most/scripts/standardize.py already handles (organ="rada" instead of
"zastupitelstvo", same dataset id 8). This file is most/'s standardize.py ported with city-slug
constants changed (ORG_ID/NAME, id prefix "most-rada:" instead of "most:") — the underlying
schema, vocabulary, and session-date handling are identical, confirmed directly against this
feed's own data (see sources.yml) before writing this, not assumed from the zastupitelstvo
precedent alone.

Scope boundary (matches every other city's precedent): this standardizer builds ONLY the bare
assembly-equivalent organization + membership. It deliberately does NOT build party/coalition
organizations or memberships — that's a later C4-equivalent phase needing owner sign-off (D7).
Note (see sources.yml's known_data_quality_findings): rada's own klub composition is a real
finding worth carrying into that later phase — all 10 members belong to Most's two governing-
coalition parties, no opposition presence at all, since rada IS the coalition's executive body.

Real data-quality findings to re-confirm once the full pipeline runs (per sources.yml's
not_yet_verified note — this docstring intentionally does NOT copy most/'s own findings, since
this is a different dataset that needs its own independent check, same practice as every prior
city never assuming a sibling dataset's quirks transfer):
1. **Vote vocabulary**: expected same 5-value single-letter codes as most/'s zastupitelstvo feed
   ("A"/"N"/"Z"/"–"/"X") — verify via the real standardize() run, not assumed.
2. **G5 namesake collisions**: `_build_persons` guards for this the same way every prior city's
   does (numeric-suffix disambiguation, never merge).
3. **`_KNOWN_ID_RENUMBERINGS`**: kept as an empty table — the MECHANISM Brno's Bořecký case needed
   is ported in case this feed ever exhibits the same source-side id-reissue pattern, but no case
   has been found here as of this writing.
4. **G2 cross-check**: same self-consistency signal as every zastupko-network city (no independent
   aggregate published; compare recomputed yes/no majority against the source's own `prijato`
   boolean).
"""
...
ORG_ID = "most-rada:org:rada-mesta-mostu"
ORG_NAME = "Rada města Mostu"

# schema-CZ.json's `hlas` enum has 7 values; only these 5 have ever been observed on the sibling
# most/ zastupitelstvo feed. Deliberately NOT mapping O/T — if they ever appear, fail loudly
# rather than guess a dt-standard option for "excused"/"secret ballot".
OPTION_MAP = {
    "A": "yes",
    "N": "no",
    "Z": "abstain",
...
def _build_organization(source_url: str) -> dict[str, Any]:
    return {
        ...
        "identifiers": json.dumps(
            [{"scheme": "most-rada:org_abbr", "identifier": "RMM"}], ensure_ascii=False
        ),
        "sources": json.dumps(
            [{"url": source_url, "note": "zastupko.cz rada feed, dataset id 8, term 2022-2026"}],
            ensure_ascii=False,
        ),
    }
...
# brno/scripts/standardize.py needed for a real, cited source-id reissue — not found in this
# feed's data as of this writing. Never add an entry here without an independent citation matching
# Brno's own bar; an id change with no citation must mint a new person instead (the default
# behavior with this empty).
    ...
    for z in zastupitele:
        ...
        if renumbering is not None:
            ...
            if len(colliding_ids) > 1:
                ...
                person_slug = slug
            person_id = f"most-rada:person:{person_slug}"

        id_to_person_id[z["id"]] = person_id

        identifiers = [{"scheme": "most-rada:zastupko_id", "identifier": str(z["id"])}]
        if renumbering is not None:
            identifiers.append(
                {
                    "scheme": "most-rada:zastupko_id_former",
        ...
        for alias in z.get("aliasy") or []:
            ...
            if former:
                identifiers.append({"scheme": "most-rada:former_name", "identifier": former})
...
def _build_memberships(
    ...
    for zastupko_id, dates in event_dates_by_person.items():
        ...
        memberships.append(
            {
                "id": f"most-rada:membership:{person_id.split(':', 2)[2]}:{ORG_ID}",
                "person_id": person_id,
...
def _session_date(session: dict[str, Any]) -> str:
    ...
    undocumented `datum_od`/`datum_do` shape. Neither present is a real schema break, not
...
def _build_votes_events_motions(
    ...
    for session in zasedani:
        ...
        for h in session["hlasovani"]:
            ...
            hlasovani_id = h["id"]
            vote_event_id = f"most-rada:vote-event:{hlasovani_id}"
            motion_id = f"most-rada:motion:{hlasovani_id}"
