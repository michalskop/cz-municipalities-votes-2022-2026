"""Build an all-members.dt.analyses-shaped JSON from Rada města Mostu's standard tables.
...
Rada města Mostu's `most-rada/scripts/party_affiliation.py` populates real, live `classification: "group"`
organizations/memberships directly from the feed's dated `politickeSubjekty[]` intervals — the same
mechanism Brno's party_affiliation.py uses (D7's preferred case), not Ostrava's per-vote klub
grouping. This script reads those into `memberships.groups` only — no dual-write into
`memberships.candidate_list`, since that field is meant for genuine candidate-list-origin data,
which this isn't.

Output is NOT committed (most-rada/work/ is gitignored) — regenerate on demand before running an
