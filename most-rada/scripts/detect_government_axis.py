"""Detect which WPCA dimension is the government/opposition axis for Rada města Mostu.

Mirrors praha/scripts/detect_government_axis.py exactly (already city-agnostic by construction —
only this docstring differs). See that file for the full method explanation (point-biserial
correlation between each dimension's per-person values and government membership, derived from
`most-rada/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`most-rada/data/memberships.csv`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`most-rada/analyses/wpca/outputs/government_axis.json`.

Usage:
  python most-rada/scripts/detect_government_axis.py
"""
