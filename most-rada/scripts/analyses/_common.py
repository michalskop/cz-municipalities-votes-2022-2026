"""Shared helpers for Rada města Mostu's analysis runners (most-rada/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per most-rada/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`most-rada/data/votes.csv` and `most-rada/data/vote_events.json` are always already present (written by
`most-rada/scripts/standardize.py`).
"""
