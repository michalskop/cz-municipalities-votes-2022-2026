"""Run the rebelity analysis for Rada města Mostu.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `most-rada/analyses/rebelity/rebelity_definition.json`.
Depends on the real `most-rada:org:group:*` data from `most-rada/scripts/party_affiliation.py` (live klub
membership, not a candidate-list fallback) via `most-rada/scripts/build_all_members.py`'s "groups"
field — no political judgment needed for rebelity itself (see the definition's own approval_status).

Usage:
  python most-rada/scripts/analyses/run_rebelity.py --script /path/to/legislature-data-analyses/rebelity/rebelity.py
"""
