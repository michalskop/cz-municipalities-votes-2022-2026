"""Run the attendance analysis for Rada města Mostu.

Invokes the shared, unmodified `attendance.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `most-rada/analyses/attendance/attendance_definition.json`
(no political judgment needed — see that file's own approval_status).

Usage:
  python most-rada/scripts/analyses/run_attendance.py --script /path/to/legislature-data-analyses/attendance/attendance.py
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-build-persons",
        action="store_true",
        help="reuse the existing --persons file instead of regenerating it from most-rada/data/",
    )
