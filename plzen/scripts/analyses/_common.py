"""Shared helpers for Plzeň's analysis runners (plzen/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per plzen/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`plzen/data/votes.csv` and `plzen/data/vote_events.json` are always already present (written by
`plzen/scripts/standardize.py`).
"""
