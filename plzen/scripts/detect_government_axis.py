"""Detect which WPCA dimension is the government/opposition axis for Plzeň.
...
`plzen/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`plzen/data/memberships.csv`).

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`plzen/analyses/wpca/outputs/government_axis.json`.

Usage:
  python plzen/scripts/detect_government_axis.py
"""
