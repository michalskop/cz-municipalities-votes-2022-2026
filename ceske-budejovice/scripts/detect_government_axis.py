"""Detect which WPCA dimension is the government/opposition axis for Zastupitelstvo statutárního města České Budějovice.
...
`ceske-budejovice/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`ceske-budejovice/data/memberships.csv`). Ported from Hradec Králové's copy, so it also unions in
`government_members` — but České Budějovice's D7-approved govity_definition.json has
`government_members: []`. The continuing governing bloc is fully covered by whole-klub
`government_groups` (ods + nezarazeni + nase-cesko + kdu-top09 + jihocesi-2012 — the three ODS/
NEZAŘAZENÍ/NAŠE ČESKO labels the same 14 councillors wore across the term, Piráti excluded per
D7), so that code path is dormant here. Kept as-is for consistency with the other cities' copies.

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`ceske-budejovice/analyses/wpca/outputs/government_axis.json`.

Usage:
  python ceske-budejovice/scripts/detect_government_axis.py
"""
