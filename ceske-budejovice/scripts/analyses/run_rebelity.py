"""Run the rebelity analysis for Zastupitelstvo statutárního města České Budějovice.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `ceske-budejovice/analyses/rebelity/rebelity_definition.json`.
Depends on the real `ceske-budejovice:org:group:*` data from `ceske-budejovice/scripts/party_affiliation.py` (live klub
membership, not a candidate-list fallback) via `ceske-budejovice/scripts/build_all_members.py`'s "groups"
field — no political judgment needed for rebelity itself (see the definition's own approval_status).

Usage:
  python ceske-budejovice/scripts/analyses/run_rebelity.py --script /path/to/legislature-data-analyses/rebelity/rebelity.py
"""
