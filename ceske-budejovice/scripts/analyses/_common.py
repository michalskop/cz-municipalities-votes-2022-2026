"""Shared helpers for Zastupitelstvo statutárního města České Budějovice's analysis runners (ceske-budejovice/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per ceske-budejovice/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`ceske-budejovice/data/votes.csv` and `ceske-budejovice/data/vote_events.json` are always already present (written by
`ceske-budejovice/scripts/standardize.py`).
"""
