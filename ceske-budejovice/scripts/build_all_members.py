"""Build an all-members.dt.analyses-shaped JSON from Zastupitelstvo statutárního města České Budějovice's standard tables.
...
Zastupitelstvo statutárního města České Budějovice's `ceske-budejovice/scripts/party_affiliation.py` populates real,
live `classification: "group"` organizations/memberships directly from the feed's dated
`politickeSubjekty[]` intervals — the same mechanism Brno's and Most's party_affiliation.py use
(D7's preferred case), not Plzeň's/Ústí's/Most-rada's per-vote mode-per-meeting derivation. This
...
Output is NOT committed (ceske-budejovice/work/ is gitignored) — regenerate on demand before running an
