"""Detect which WPCA dimension is the government/opposition axis for Zastupitelstvo města Ústí nad Labem.

Mostly mirrors praha/scripts/detect_government_axis.py (point-biserial correlation between each
dimension's per-person values and government membership, derived from
`usti-nad-labem/analyses/govity/govity_definition.json`'s `government_groups` expanded via
`usti-nad-labem/data/memberships.csv`) -- WITH ONE DELIBERATE DIFFERENCE: this copy also unions in
`government_members` (named individual person ids), not just `government_groups` (whole-klub org
ids). Every other city's copy of this script (praha/brno/ostrava/most/plzen/most-rada) only expands
government_groups, because none of them has ever needed a non-empty government_members list.
Ústí's D7-approved govity_definition.json is the first to actually use government_members (2 rada
members -- Nechybová, Tošovský -- are formally 'nezařazení', not covered by a klub-level group), so
the original groups-only logic would have silently excluded them from the WPCA axis-detection
government label, contradicting govity.py itself (which already reads both fields, see
legislature-data-analyses/govity/govity.py's `is_in_government()`). Fixed here to match govity.py's
own semantics; not backported to other cities since their government_members lists are empty and
the bug is otherwise dormant.

Output: a sidecar JSON, NOT a change to wpca.json itself. Written to
`usti-nad-labem/analyses/wpca/outputs/government_axis.json`.

Usage:
  python usti-nad-labem/scripts/detect_government_axis.py
"""
