"""Build an all-members.dt.analyses-shaped JSON from Zastupitelstvo města Ústí nad Labem's standard tables.
...
Zastupitelstvo města Ústí nad Labem's `usti-nad-labem/scripts/party_affiliation.py` populates real,
live `classification: "group"` organizations/memberships derived from the per-vote klub text
captured in every meeting PDF (mode klub per meeting, contiguous-run date intervals) — the same
mechanism Plzeň's and Most-rada's party_affiliation.py use, not Brno's dated `politickeSubjekty[]`
feed intervals. This script reads those into `memberships.groups` only — no dual-write into
`memberships.candidate_list`, since that field is meant for genuine candidate-list-origin data,
which this isn't.

Output is NOT committed (usti-nad-labem/work/ is gitignored) — regenerate on demand before running an
