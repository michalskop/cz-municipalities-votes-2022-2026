"""Run the rebelity analysis for Zastupitelstvo města Ústí nad Labem.

Invokes the shared, unmodified `rebelity.py` from the separate `legislature-data-analyses`
repository. City-specific content is entirely in `usti-nad-labem/analyses/rebelity/rebelity_definition.json`.
Depends on the real `usti-nad-labem:org:group:*` data from `usti-nad-labem/scripts/party_affiliation.py` (live klub
membership, not a candidate-list fallback) via `usti-nad-labem/scripts/build_all_members.py`'s "groups"
field — no political judgment needed for rebelity itself (see the definition's own approval_status).

Usage:
  python usti-nad-labem/scripts/analyses/run_rebelity.py --script /path/to/legislature-data-analyses/rebelity/rebelity.py
"""
