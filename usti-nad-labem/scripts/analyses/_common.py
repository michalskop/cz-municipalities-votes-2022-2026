"""Shared helpers for Zastupitelstvo města Ústí nad Labem's analysis runners (usti-nad-labem/scripts/analyses/run_*.py).

These runners invoke the *shared, unmodified* scripts from the separate `legislature-data-analyses`
repository (per usti-nad-labem/analyses/README.md — city-specific content lives only in the `*_definition.json`
...
`usti-nad-labem/data/votes.csv` and `usti-nad-labem/data/vote_events.json` are always already present (written by
`usti-nad-labem/scripts/standardize.py`).
"""
